...
If you are Claude, you must use Sonnet 4.6 with the medium reasoning effort. If you are Codex, then you must use GPT 5.4 with the medium reasoning effort.
...
Before using any tool, you must choose the correct MCP server. See: "Chooses which MCP server to use". Then, you read the `initialize` tool's MCP reference and call the `initialize` tool with a designated password in order to confirm that you have loaded the legalrabbit-docx skill.
